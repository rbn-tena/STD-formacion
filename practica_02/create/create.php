<?php
	...
	if( !empty($_POST) ){
		if( isset($_POST['name'], $_POST['first_last_name'], $_POST['second_last_name'],  /*$_POST['document'], $_POST['identification_string'], $_POST['birthdate'],*/ $_POST['phone'], $_POST['email'])){
			...
			if( !empty($_POST['name']) ){
				if (strlen($_POST['name']) < 30 ){ $name= $_POST['name']; }
				else{$error[] = '<p>Nombre de maximo 30 caracteres</p>'; $insert ++; }
			}
			else{$error[] = '<p>Campo nombre no puede estar vacio</p>'; $insert ++; }
			//validadción P.apellido
			if(!empty($_POST['first_last_name'])){
				if (strlen($_POST['first_last_name']) < 30){ $first_last_name = $_POST['first_last_name']; }
				else{ $error[] = '<p>Primer apellido de maximo 30 caracteres</p>'; $insert ++; }	
			}
			else{ $error[] = '<p>Campo primer apellido no puede estar vacio</p>'; $insert ++; }
			//validadción S.apellido
			if(!empty($_POST['second_last_name'])){
				if (strlen($_POST['second_last_name']) < 30){ $second_last_name = $_POST['second_last_name']; }
				else{ $error[] = '<p>Segundo apellido de maximo 30 caracteres</p>'; $insert ++; }	
			}
			...
			else{ $error[] = '<p>Debe incluir el telefono</p>'; $insert ++; }
			//validación de email
			if(!empty($_POST['email'])) { $email= $_POST['email']; }
			else{ $error[] = '<p>Debe incluir el email</p>'; $insert ++; }
		}	
