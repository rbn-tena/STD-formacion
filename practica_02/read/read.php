<?php
	...
	if(!empty($_POST['name'])){
		if ($contador_where > 0){ $where .= " AND name = '".$_POST['name']."'"; }
		$where ="WHERE name = '".$_POST['name']."'";
		$contador_where ++;
	}
	
	if(!empty($_POST['first_last_name'])){
		if($contador_where > 0){ $where .= " AND first_last_name = '".$_POST['first_last_name']."'"; }
		$where ="WHERE first_last_name = '".$_POST['first_last_name']."'";
		$contador_where ++;
	}
	
	if(!empty($_POST['second_last_name'])){
		if($contador_where > 0){ $where .= " AND second_last_name = '".$_POST['second_last_name']."'"; }
		$where ="WHERE second_last_name = '".$_POST['second_last_name']."'";
		$contador_where ++;
	}
	
	if(!empty($_POST['phone'])){
		if($contador_where > 0){ $where .= " AND phone = '".$_POST['phone']."'"; }
		$where ="WHERE phone = '".$_POST['phone']."'";
		$contador_where ++;
	}
	
	if(!empty($_POST['email'])){
		if($contador_where > 0){ $where .= " AND email = '".$_POST['email']."'"; }
		$where ="WHERE email = '".$_POST['email']."'";
		$contador_where ++;
	}
	
	if($contador_where > 0){ $sql .= $where; }
