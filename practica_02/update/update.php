<?php
	...
	if(!empty($_POST['first_last_name'])){
		if ($contador_set > 0){ $set .= " , `first_last_name` = '".$_POST['first_last_name']."'"; }
		$set =" `first_last_name` = '".$_POST['first_last_name']."'";
		$contador_set ++;
	}
	
	if(!empty($_POST['second_last_name'])){
		if ($contador_set > 0){ $set .= " , `second_last_name` = '".$_POST['second_last_name']."'"; }
		$set =" `second_last_name` = '".$_POST['second_last_name']."'";
		$contador_set ++;
	}
	
	if(!empty($_POST['phone'])){
		if($contador_set > 0){ $set .= " , `phone` = '".$_POST['phone']."'"; }
		$set =" phone = '".$_POST['phone']."'";
		$contador_set ++;
	}
	
	if(!empty($_POST['email'])){
		if($contador_set > 0){ $set .= " , `email` = '".$_POST['email']."'"; }
